...
export default function Dashboard(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    async function fetchBalance() {
      try {
        const rawBalance = await ethProvider.getBalance(account);
        const formatted = Number(ethers.formatEther(rawBalance)).toFixed(4);
        if (!stale) setBalance(formatted);
      } catch (e) {
    ...
    // Listen for new blocks for live updates
    ethProvider.on("block", fetchBalance);

    // Listen for account/network changes (if provider supports it)
    if ((window as any).ethereum?.on) {
      (window as any).ethereum.on("accountsChanged", fetchBalance);
      (window as any).ethereum.on("chainChanged", fetchBalance);
    }

    return () => {
      stale = true;
      ethProvider.off("block", fetchBalance);
      if ((window as any).ethereum?.removeListener) {
        (window as any).ethereum.removeListener(
          "accountsChanged",
          fetchBalance
        );
        (window as any).ethereum.removeListener("chainChanged", fetchBalance);
      }
    };
  }, [account]);
